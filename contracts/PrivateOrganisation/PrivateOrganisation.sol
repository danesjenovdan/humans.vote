...
contract PrivateOrganisation is Owned {
     ...
     *
     * Propose to send `etherAmount` ether to `beneficiary` for `jobDescription`. `transactionBytecode ?
     * Contains : Does not contain` code. This is a convenience function to use if the amount to be given is
     * in round number of ether units.
     *
     * @param beneficiary who to send the ether to
    ...
    function vote(
        ...
        returns (uint voteID)
    {
        Proposals storage p = proposals[proposalNumber];         // Get the proposal
